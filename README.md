...
Walkplay EQ 让你在 Windows 桌面上直接调节硬件耳放的 10 段参量均衡——拖动滑块即时写入设备，并以实时频响曲线可视化效果。基于 [Tauri 2](https://tauri.app/) + React 构建，原生、轻量、无需常驻浏览器内核之外的额外运行时。

<p align="center">
  <img src="docs/screenshots/hero.png" alt="Walkplay EQ — 参数均衡器主界面" width="860">
</p>

<p align="center">
  <sub>实时频响曲线 · 10 段参量均衡 · 预设切换器 · 前置增益 · 内置简体中文 / English 双语界面</sub>
</p>

<details>
<summary>📷 English UI（点击展开英文界面截图）</summary>

<p align="center">
  <img src="docs/screenshots/hero-en.png" alt="Walkplay EQ — Parametric EQ (English UI)" width="860">
</p>
...
T02 使用 HID 输出报告传输 biquad（双二阶）滤波器系数：上位机将每个频段的频率、Q、增益、类型换算为定点化的滤波器系数，组成完整的 10 段程序后一次性下发并提交（commit），前置增益作为独立帧发送。相关实现见 `src/features/eq/t02-protocol.ts` 与 `crates/walkplay-dac-protocol/`，协议细节经与官方应用对比验证。
